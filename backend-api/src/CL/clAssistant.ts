...
const test_cl: CoverLetterResponse = {
    ...
    why_work_here: 'I am particularly excited about the opportunity to be mentored by experienced professionals and to work on impactful projects that align with my career goals and interests.',
...
export async function genCL(jobInfo: JobInfo|string): Promise<CoverLetterResponse> {
        ...
        if(Number(process.env.TEST) === 1) {
            ...
            // 1 - into the desired format (string)
            let inputTxt: string;
